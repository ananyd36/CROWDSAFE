...
def detect_image(input_file_path,output_path,model_def = "config/yolov3_mask.cfg",weights_path = "checkpoints/yolov3_ckpt_35.pth",class_path="data/mask_dataset.names",conf_thres = 0.8,nms_thres = 0.3,frame_size=416):

    # opt = parser.parse_args()
    # print(opt)

    file_split = output_path.split("/")
    filename_ = file_split[-1]
    print(filename_)
    # Output directory
    ...
    # for imagename in os.listdir(input_file_path):

    #     print("\n"+imagename+"_______")
    image_path = os.path.join(input_file_path)
    ...
    out_filepath = os.path.join(output_path)
    # print(out_filepath)
    cv2.imwrite(out_filepath+"/output.jpg",org_img)
